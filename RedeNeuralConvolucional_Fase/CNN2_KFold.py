...
"""
CNN Architecture: ThreeConvolutionalLayers_32-16-16
...
- BatchNormalization
- Conv1D: 16 filters, kernel_size=2, ReLU, same padding  ← EXTRA LAYER
- BatchNormalization
- Flatten
- Dense: 32 units, ReLU
- Dropout: 0.3
...
dataset_output = os.path.join(path, f"CNN_2", f'janelamento_{N_JANELAMENTO}')

#----------------------------- Data loading -------------------------------------------
data = np.load(dataset_path)

MatrizAmostras = data['MatrizAmostras']
FaseAssociada = data['FaseReal']
...
for fold, (train_index, test_index) in enumerate(kf.split(MatrizAmostras)):
    ...
    y_treino, y_teste = FaseAssociada[train_index], FaseAssociada[test_index]
    ...
    for ocupacao in ocupacoes:
        ...
        if np.sum(mask) > 0:
            ...
            indices_locais = indices_locais_global[indices_globais]

            results_ocupacoes[ocupacao]['error'].extend(error[mask])
            results_ocupacoes[ocupacao]['real_fase'].extend(y_teste[mask])
            results_ocupacoes[ocupacao]['estimated_fase'].extend(y_pred[mask])
...
for ocupacao in ocupacoes:
    if len(results_ocupacoes[ocupacao]['error']) > 0:
        ...
        real_fase_array = np.array(results_ocupacoes[ocupacao]['real_fase'], dtype=np.float64)
        estimated_fase_array = np.array(results_ocupacoes[ocupacao]['estimated_fase'], dtype=np.float32)
        ...
        np.savez(
            ...
            error=error_array,
            real_fase=real_fase_array,
            estimated_fase=estimated_fase_array,
