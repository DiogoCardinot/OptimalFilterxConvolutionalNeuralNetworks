...
"""
CNN Architecture: SigmoidActivation_32-16
- Input: (7, 1) - Reshape from 7 features
- Conv1D: 32 filters, kernel_size=3, Sigmoid, same padding  ← SIGMOID
- BatchNormalization
- Conv1D: 16 filters, kernel_size=2, Sigmoid, same padding  ← SIGMOID
- BatchNormalization
- Flatten
- Dense: 32 units, Sigmoid  ← SIGMOID
- Dropout: 0.3
...
dataset_output = os.path.join(path, f"CNN_6",  f'janelamento_{N_JANELAMENTO}')
...
for fold, (train_index, test_index) in enumerate(kf.split(MatrizAmostras)):
    ...
    for ocupacao in ocupacoes:
        ...
        if np.sum(mask) > 0:
            ...
            results_ocupacoes[ocupacao]['real_fase'].extend(y_teste[mask].tolist())
            results_ocupacoes[ocupacao]['estimated_fase'].extend(y_pred[mask].tolist())
