...
def load_data_for_occupation(occupation):
    ...
    for i in range(1, n_cnns+1):
        cnn_path = os.path.join(base_path,f'RedeNeuralConvolucional_Fase', f"CNN_{i}", f'janelamento_{n_janelamento}' ,f"results_ocupacao_{occupation}.npz")
...
def plot_heatmaps_individual_colorbar():
    ...
    for idx, occupation in enumerate(occupations):
        ...
        axes[idx].set_xticklabels(['RMS', 'R²', 'MAE', 'MedAE'], fontsize=fontSize-2)
        axes[idx].set_yticklabels(cnn_names, fontsize=fontSize-2)
        ...
        cbar.set_label('Magnitude', fontsize=fontSize-2, rotation=270, loc='center', labelpad=30)
...
############################################################ CNNS ESPECIFICAS, NÃO TODAS ###############################################

def load_data_for_occupation_cnns(occupation, cnns):
    """Carrega dados para uma ocupação específica"""
    data_dict = {}
    results_ocupacao = 'results_ocupacao'
    for cnn in cnns:
        cnn_path = os.path.join(base_path,f'RedeNeuralConvolucional_Fase', f"CNN_{cnn}", f'janelamento_{n_janelamento}',f"{results_ocupacao}_{occupation}.npz")
        cnn_data = np.load(cnn_path)
        data_dict[f'CNN_{cnn}'] = {
            'rms': cnn_data['rms'],
            'r2': cnn_data['r2'], 
            'mae': cnn_data['mae'],
            'medae': cnn_data['medae']
...
def create_heatmap_data_cnns(data_dict, cnns):
    """Cria matriz para o heatmap: 7 CNNs x 4 métricas"""
    cnn_names = [f'CNN-{cnn}' for cnn in cnns]
    metrics = ['rms', 'r2', 'mae', 'medae']
    n_cnns = len(cnns)
    heatmap_data = np.zeros((n_cnns, 4))
...
def plot_heatmaps_individual_colorbar_cnns(cnns):
    """Plota os 4 heatmaps com colorbar individual para cada um"""
    fig, axes = plt.subplots(2, 2, figsize=(20, 12))
    axes = axes.flatten()
    ...
    for idx, occupation in enumerate(occupations):
        data_dict = load_data_for_occupation_cnns(occupation, cnns)
        heatmap_data, cnn_names, metrics = create_heatmap_data_cnns(data_dict, cnns)
...
cnns = [3,5,8]
plot_heatmaps_individual_colorbar_cnns(cnns)
